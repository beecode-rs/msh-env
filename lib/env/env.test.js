...
describe.each([
    ...
    describe('_envNames', () => {
        it('should call names of naming strategy', () => {
            ...
            assert_1.default.deepEqual(result, ['test1', ...dummyEnvNames]);
        });
        it('should simulate double prefixing', () => {
            const fakePrefixFactory = (prefix) => (names) => names.map((name) => [prefix, name].join(''));
            ...
            expect(mockNamingStrategy2.names).toHaveBeenCalledWith(dummyEnvNames
                .slice()
                .reverse()
